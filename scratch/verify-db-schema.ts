...
const prisma = new PrismaClient({
  datasources: {
    db: {
      url: "postgresql://dressme_baseline_admin@localhost:55432/dressme_canonical_baseline",
    },
  },
});

async function main() {
  console.log("Connected to dressme_canonical_baseline via Prisma");
  ...
  `);
  console.log("\n=== Tables ===");
  console.log(tablesRes.map((r: any) => r.table_name).join(", "));

  // 2. Check enums
  const enumsRes: any = await prisma.$queryRawUnsafe(`
  ...
  `);
  console.log("\n=== Enums ===");
  for (const row of enumsRes) {
    console.log(`${row.enum_name}: [${row.values}]`);
  }

  // 3. Check obsolete columns
  const obsoleteColsRes: any = await prisma.$queryRawUnsafe(`
  ...
  console.log(obsoleteColsRes);

  // 4. Check ProductVariant columns and nullability / defaults
  const pvColsRes: any = await prisma.$queryRawUnsafe(`
  ...
  console.table(pvColsRes);

  // 5. Check ProductCategory primary key and columns
  const pcColsRes: any = await prisma.$queryRawUnsafe(`
    SELECT column_name, is_nullable, column_default, data_type
    FROM information_schema.columns 
  ...
  console.table(pcColsRes);

  // 6. Check Foreign Keys on ProductVariant & ProductCategory
  const fkRes: any = await prisma.$queryRawUnsafe(`
    SELECT
      tc.table_name, 
      kcu.column_name, 
