...
interface Product { id: string; name: string; slug: string; description: string; price: number; status: string; brand: { name: string }; categories: Array<{ name: string }> ; vendor: { businessName: string }; variants: Array<{ id: string; sizeValue: string; colorValue: string; stock: number; price?: number | null }>; }
...
export function CustomerWorkspace() {
  ...
  return <PageFrame><Stack spacing={3}><Box><Typography component="h1" variant="h4">Customer commerce workspace</Typography><Typography color="text.secondary">Live catalogue, cart, favourites, and review operations.</Typography><Button component={RouterLink} to={ROUTES.customerCart} sx={{ mt: 1 }}>View my cart</Button></Box><TextField label="Search products" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name or description" />
    {products.isLoading && <CircularProgress />}{products.isError && <Alert severity="error" action={<Button onClick={() => products.refetch()}>Retry</Button>}>{message(products.error)}</Alert>}
     <Grid container spacing={2}>{products.data?.items.map((product) => <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={product.id}><Card variant="outlined"><CardContent><Stack spacing={1}><Typography variant="h6">{product.name}</Typography><Typography variant="body2" color="text.secondary">{product.brand?.name || "Brand"} · {(product.categories ?? []).map((category) => category.name).join(", ") || "Uncategorized"} · {product.vendor?.businessName || "Vendor"}</Typography><Typography>{product.description}</Typography><Typography fontWeight={700}>{formatCurrency(product.price)}</Typography><Chip size="small" label={`${(product.variants ?? []).reduce((sum: number, v: any) => sum + (v.stock ?? 0), 0)} in stock · ${product.status}`} sx={{ alignSelf: "start" }} /></Stack></CardContent><CardActions><Button disabled={cart.isPending} onClick={() => cart.mutate({ productId: product.id, variantId: product.variants[0]?.id })}>Add to cart</Button><IconButton disabled={favourite.isPending} onClick={() => favourite.mutate(product.id)} aria-label="Add favourite"><FavoriteBorderIcon /></IconButton><Button onClick={() => setSelected(product)}>Review</Button></CardActions></Card></Grid>)}</Grid>
